...
foreach ($chromeDataPath in $foundPaths) {
    ...
    foreach ($profile in $profiles) {
        ...
        if (-not $KeepPasswords) {
            Write-Host "Clearing ALL password and autofill data..." -ForegroundColor White
            
            # Primary password databases
            Remove-SafeFiles $profilePath @("Login Data", "Login Data-journal", "Login Data-shm", "Login Data-wal") "Saved passwords (primary)"
            Remove-SafeFiles $profilePath @("Password Manager") "Password manager data"
            
            # Web Data contains autofill suggestions (usernames, addresses, etc.)
            Remove-SafeFiles $profilePath @("Web Data", "Web Data-journal", "Web Data-shm", "Web Data-wal") "Web data and autofill suggestions"
            
            # Additional password-related files
            Remove-SafeFiles $profilePath @("Account Info") "Account information"
            Remove-SafeFiles $profilePath @("Affiliation Database", "Affiliation Database-journal") "Password affiliation data"
            
            # Clear preferences that store password and autofill settings
            Write-Host "Clearing password preferences..." -ForegroundColor White
            if (Test-Path "$profilePath\Preferences") {
                try {
                    $prefsContent = Get-Content "$profilePath\Preferences" -Raw
                    if ($prefsContent) {
                        # Remove password manager preferences
                        $prefsContent = $prefsContent -replace '"password_manager":\{[^}]*\},?', ''
                        $prefsContent = $prefsContent -replace '"credentials_enable_service":[^,}]*,?', ''
                        $prefsContent = $prefsContent -replace '"profile":\{"password_manager":[^}]*\}', '"profile":{}'
                        $prefsContent = $prefsContent -replace '"autofill":\{[^}]*\},?', ''
                        
                        # Clean up malformed JSON
                        $prefsContent = $prefsContent -replace ',\s*}', '}'
                        $prefsContent = $prefsContent -replace ',\s*]', ']'
                        
                        Set-Content "$profilePath\Preferences" -Value $prefsContent -Encoding UTF8
                        Write-Host "[OK] Cleared password preferences" -ForegroundColor Green
                    }
                } catch {
                    # If preferences parsing fails, just remove the file
                    Write-Host "[INFO] Removing entire preferences file due to parsing error" -ForegroundColor Yellow
                    Remove-SafeFiles $profilePath @("Preferences") "Preferences (fallback)"
                }
            }
            ...
            # Clear any cached credential data from local storage
            Write-Host "Clearing credential storage..." -ForegroundColor White
            if (Test-Path "$profilePath\Local Storage\leveldb") {
                Remove-SafeDirectory "$profilePath\Local Storage\leveldb" "Local storage credentials" @("Extensions")
            }
            
            # Final verification - check if critical files still exist
            Write-Host "Verifying password deletion..." -ForegroundColor White
            $criticalFiles = @("Login Data", "Web Data", "Preferences")
            $remainingFiles = @()
            
            foreach ($file in $criticalFiles) {
                if (Test-Path "$profilePath\$file") {
                    $remainingFiles += $file
                }
            }
            
            if ($remainingFiles.Count -gt 0) {
                Write-Host "[WARNING] Some files still exist: $($remainingFiles -join ', ')" -ForegroundColor Yellow
                # Try to force delete remaining files
                foreach ($file in $remainingFiles) {
                    $filePath = "$profilePath\$file"
                    try {
                        Remove-Item $filePath -Force -ErrorAction SilentlyContinue
                        if (-not (Test-Path $filePath)) {
                            Write-Host "[OK] Force deleted: $file" -ForegroundColor Green
                        }
                    } catch {
                        Write-Warning "Could not force delete $file"
                    }
                }
            } else {
                Write-Host "[VERIFIED] All critical password files successfully removed" -ForegroundColor Green
            }
            
            Write-Host "[SUCCESS] All password and autofill data cleared" -ForegroundColor Green
            
        } else {
            Write-Host "[PRESERVED] Keeping saved passwords" -ForegroundColor DarkGreen
            # Still clear form data but keep passwords
            Write-Host "Clearing form autofill data only..." -ForegroundColor White
            Remove-SafeFiles $profilePath @("Web Data", "Web Data-journal", "Web Data-shm", "Web Data-wal") "Form autofill data"
        }
